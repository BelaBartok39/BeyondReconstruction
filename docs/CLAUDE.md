...
**IMPORTANT:** The `main` branch is reserved for the **v1 production model** and the IEEE paper we are writing on it. All files on `main` should be directly relevant to the v1 production pipeline.

- **All new experiments** (beyond v1) must be done on **feature branches** (e.g., `experiment/b2-snr-range`, `experiment/vq-vae`)
- **Do not commit** experimental checkpoints, configs, or scripts to `main` unless they become part of the v1 paper
- V1 production pipeline: `experiments/train_baseline.py` → `experiments/test_improved_detection.py`
- V2 reproduction pipeline: `experiments/reproduce_production.py` (written later, NOT part of v1)
